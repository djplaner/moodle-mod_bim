...
function show_marker_student_details( $bim, $userid, $cm ) {
    ...
    if ( $bim->register_feed == 0 || $bim->mirror_feed == 0 ) {
        echo $OUTPUT->heading(get_string('marker_student_config_heading', 'bim'), 2 );
...
function bim_setup_posts_table( $cm, $bim, $userid, $questions ) {
    ...
    $columns = array( "username", "name", "questions", "marked" );
        ...
        if ( isset( $feed_details[$user->id] )) {
            ...
            foreach ($questions as $bim_qid => $question) {
                ...
                foreach ($stud_marking_details as $row) {
                    ...
                    if ( ! isset( $questions[$row->question] ) ) {
                        continue;
                    }
                    if ( $qid == $bim_qid ) {
...
function bim_create_details_display( $user_details, $feed_details=null, $cm = null ) {
    global $CFG;
    ...
    foreach ($user_details as $user) {
        ...
        if ( isset( $feed_details[$user->id] )) {
            ...
            if ( $lastpost > 0 ) {
                $diff = bim_time_diff( time(), $feed_details[$user->id]->lastpost );
...
function bim_marker_mark_post( $bim, $userid, $cm, $marking ) {
    ...
    $next_prev_q = bim_get_next_prev_question( $marking, $marking_details );
    $next_prev_s = bim_get_next_prev_student(
    ...
    if ( ! $marking_form->is_submitted() ) {
        ...
        if ( $allocation_change ) {
            ...
            if ( $fromform->$menu_name == "Unallocate" ) {
                ...
                // re-allocating to a different question
